...
function setUser(lat,lng,label='Lokasi Anda'){userLatLng={lat,lng};if(userMarker)map.removeLayer(userMarker);userMarker=L.circleMarker([lat,lng],{radius:8,color:'#172033',fillColor:'#fff',fillOpacity:1,weight:4}).addTo(map).bindTooltip(label);map.setView([lat,lng],14);renderNearest();}
...
function clearRoute(){if(routeLayer){map.removeLayer(routeLayer);routeLayer=null}if(isoLayer){map.removeLayer(isoLayer);isoLayer=null}document.getElementById('routeCard').classList.add('hidden')}
...
function setAccidentPoint(lat,lng){resetDispatchPanel();accidentPickMode=false;document.body.classList.remove('map-pick-cursor');previousUserLatLng=previousUserLatLng||userLatLng;userLatLng={lat,lng};if(userMarker){map.removeLayer(userMarker);userMarker=null;}if(accidentMarker)map.removeLayer(accidentMarker);accidentMarker=L.marker([lat,lng],{icon:victimIcon(),zIndexOffset:1200}).addTo(map).bindPopup('<b>🚨 Titik Korban Kecelakaan</b><br>Lokasi simulasi insiden<br><small>Menjadi titik awal perhitungan fasilitas terdekat dan routing.</small>').openPopup();map.setView([lat,lng],15);renderNearest();nearestMedicalFeature=nearestByCategory(lat,lng,'Medis');nearestPoliceFeature=nearestByCategory(lat,lng,'Keamanan');document.getElementById('victimCoords').textContent=`${lat.toFixed(6)}, ${lng.toFixed(6)}`;if(nearestMedicalFeature){document.getElementById('nearestMedicalName').textContent=nearestMedicalFeature.f.properties.nama;document.getElementById('nearestMedicalDistance').textContent=`± ${nearestMedicalFeature.d.toFixed(1)} km garis lurus`;}if(nearestPoliceFeature){document.getElementById('nearestPoliceName').textContent=nearestPoliceFeature.f.properties.nama;document.getElementById('nearestPoliceDistance').textContent=`± ${nearestPoliceFeature.d.toFixed(1)} km garis lurus`;}document.getElementById('incidentPanel').classList.remove('hidden');document.getElementById('incidentModeBadge').classList.remove('hidden');document.getElementById('incidentBadgeText').textContent='Titik korban aktif · klik Rute untuk respons';document.getElementById('accidentModal').classList.add('hidden');}
function endAccidentSimulation(){resetDispatchPanel();if(accidentMarker){map.removeLayer(accidentMarker);accidentMarker=null;}accidentPickMode=false;document.body.classList.remove('map-pick-cursor');document.getElementById('incidentPanel').classList.add('hidden');document.getElementById('incidentModeBadge').classList.add('hidden');if(routeLayer){map.removeLayer(routeLayer);routeLayer=null}if(isoLayer){map.removeLayer(isoLayer);isoLayer=null}document.getElementById('routeCard').classList.add('hidden');selected=null;nearestMedicalFeature=null;nearestPoliceFeature=null;if(previousUserLatLng){setUser(previousUserLatLng.lat,previousUserLatLng.lng,'Lokasi Anda')}else{userLatLng=null;renderNearest()}previousUserLatLng=null;}
...
document.getElementById('routeNearestMedical')?.addEventListener('click',()=>{if(nearestMedicalFeature)routeTo(nearestMedicalFeature.f.properties.id)});
document.getElementById('routeNearestPolice')?.addEventListener('click',()=>{if(nearestPoliceFeature)routeTo(nearestPoliceFeature.f.properties.id)});
document.getElementById('endAccidentSimulation')?.addEventListener('click',endAccidentSimulation);
document.getElementById('closeIncidentPanel')?.addEventListener('click',()=>document.getElementById('incidentPanel').classList.add('hidden'));

// v10 quick feature tiles
document.querySelectorAll('.feature-tile').forEach(btn=>{
  btn.addEventListener('click',()=>{
    const f=btn.dataset.feature;
    if(f==='incident'){ document.getElementById('accidentBtn')?.click(); return; }
    if(f==='nearest'){ document.getElementById('locBtn')?.click(); return; }
    if(f==='route'){
      const first=document.querySelector('#nearestList button, .facility-card button');
      if(first) first.click();
      else alert('Aktifkan Lokasi Saya terlebih dahulu untuk menampilkan fasilitas terdekat.');
    }
  });
});
