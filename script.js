// ===== GitHub Pages-safe local asset paths =====
// Keeps the same project working in VS Code Live Server and GitHub Pages.
...
async function loadDamkar(){const note=document.getElementById('damkarNotice');try{const r=await fetch(assetURL('data/damkar_semarang.json'),{cache:'no-store'});if(!r.ok)throw Error('HTTP '+r.status);const arr=await r.json();let ok=0,failed=0;for(const d of arr){let lat=d.lat,lng=d.lng;if(lat==null||lng==null){try{const g=await geocodeDamkar(d);lat=g.lat;lng=g.lng;await sleep(1100)}catch(e){failed++;continue}}features.push(toFeature(d,lat,lng));ok++;}renderMarkers();note.textContent=`🔥 ${ok} pos Damkar aktif di peta${failed?`; ${failed} titik belum memiliki koordinat`:''}.`;note.classList.add('success')}catch(e){note.textContent='🔥 Data Damkar belum dapat dimuat; kategori lain tetap dapat digunakan.'}}

async function loadMainFacilities(){
  try{
    const r=await fetch(assetURL('data/fasilitas_semarang.geojson'),{cache:'no-store'});
    if(!r.ok)throw Error('HTTP '+r.status);
    const j=await r.json();
    if(!Array.isArray(j.features))throw Error('Format GeoJSON tidak valid');
    features=j.features;
    renderMarkers();
  }catch(e){
    console.error('Gagal memuat fasilitas:',e);
    if(list)list.innerHTML='<p>Data fasilitas belum dapat dimuat.</p>';
  }
}

// Muat independen agar kegagalan boundary/Damkar tidak menghilangkan marker fasilitas.
loadMainFacilities();
loadBoundary();
loadDamkar();
